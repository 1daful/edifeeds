  ...
  <div class="text-h4 q-mb-md q-pa-md row media item-start q-gutter-md" :key="type">
    <template v-if="mediaItems.length > 0">
      <q-card class="col-2 myCard" v-for="mediaItem in mediaItems" :key="mediaItem.id">
        <q-img
          :src="mediaItem[type].thumbnaillarge"
          spinner-color="white"
          style="height: 15em"
        >
        ...
        <q-card-section>
          ...
          >
            <p class="text-weight-bold" style="font-size: 16px">
              {{ mediaItem[type].title }}
            </p>
            <q-item-label class="caption text-subtitle1"
              >{{ mediaItem[type].description?.slice(0, 110) }}
              <span v-if="mediaItem[type].description?.length > 180"
                >...</span
              ></q-item-label
            >
          </router-link>
        </q-card-section>
        <q-card-actions v-if="mediaItem[type].inserted_at">
...
export default defineComponent({
  ...
  async mounted() {
    ...
    let collectedItems = await repository.readItems(
      coll,
      { key: "*", fColl: this.type, fKey1: `*`, fKey2: "userId" },
      undefined,
      this.user?.id,
      10
    );
