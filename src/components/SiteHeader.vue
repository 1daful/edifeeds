    ...
    <q-toolbar class="header text-h5 text-weight-bold justify-between">
      ...
      <div class="row justify-end q-ml-md ml-auto gt-sm" v-if="!userInfo">
        <q-btn
          ...
          size="17px"
          outline
          class="outline rounded-borders q-ma-sm"
          color="primary"
          :to="{ name: 'SignIn', params: { myUrl: '/home' } }"
        >
          Sign in
        </q-btn>
      ...
      <q-avatar v-if="userInfo" color="primary">
      ...
      <q-btn v-if="userInfo" color="primary" dense push size="17px" @click="signout"
        >Sign out</q-btn
      >
...
export default defineComponent({
  ...
  methods: {
    ...
    getMenus(routes: RouteRecordRaw[]) {
      for (let route of routes) {
        if (route.children) {
          if (route.meta?.navigational) this.menus.push(route);
          this.getMenus(route.children);
        } else {
  ...
  mounted() {
    this.getMenus(this.$router.options.routes);
